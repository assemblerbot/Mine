...
public sealed class ImGuiRenderer : IDisposable
{
    ...
    private ImGuiColorSpaceHandling _colorSpaceHandling;
    ...
    internal ImGuiRenderer(GraphicsDevice gd, OutputDescription outputDescription, int width, int height)
        : this(gd, outputDescription, width, height, ImGuiColorSpaceHandling.Legacy)
    {
    }
        ...
        ImGuiColorSpaceHandling colorSpaceHandling)
    {
        ...
        byte[] vertexShaderBytes =
            LoadShaderCode(gd.ResourceFactory, "imgui-vertex", ShaderStages.Vertex, _colorSpaceHandling);
        byte[] fragmentShaderBytes =
            LoadShaderCode(gd.ResourceFactory, "imgui-frag", ShaderStages.Fragment, _colorSpaceHandling);
        ...
        GraphicsPipelineDescription pd = new GraphicsPipelineDescription(
            ...
            new ShaderSetDescription(
                ...
                new[]
                {
                    ...
                    new SpecializationConstant(1, _colorSpaceHandling == ImGuiColorSpaceHandling.Legacy),
                }),
        ...
        ImGuiColorSpaceHandling colorSpaceHandling)
    {
        switch (factory.BackendType)
        {
            ...
            {
                if (stage == ShaderStages.Vertex && colorSpaceHandling == ImGuiColorSpaceHandling.Legacy)
                {
                    name += "-legacy";
                }

                string resourceName = "HLSL/" + name + ".hlsl.bytes";
            ...
            case GraphicsBackend.OpenGL:
            {
                ...
                string resourceName = "GLSL/" + name + ".glsl";
                return GetResourceBytes(resourceName);
            }
            case GraphicsBackend.OpenGLES:
            {
                ...
                string resourceName = "GLSLES/" + name + ".glsles";
                return GetResourceBytes(resourceName);
            }
            case GraphicsBackend.Vulkan:
            {
                string resourceName = "SPIR-V/" + name + ".spv";
                return GetResourceBytes(resourceName);
            }
            case GraphicsBackend.Metal:
            {
                string resourceName = "Metal/" + name + ".metallib";
                return GetResourceBytes(resourceName);
            }
            default:
    ...
    {
        return Engine.Resources.ReadResource("Plugins/ImGuiAssets/" + path) ?? Array.Empty<byte>();
    ...
    private void UpdateImGuiInput()
    {
        ...
        // key events
        foreach(InputKeyboardEvent keyboardEvent in Engine.Input.KeyboardEvents)
        {
            io.AddKeyEvent(ImGuiInputExtensions.ToImGuiKey(keyboardEvent.Key), keyboardEvent.Pressed);
        }

        // key char presses
        foreach (char charEvent in Engine.Input.KeyboardCharEvents)
        {
        ...
        {
            io.AddMouseWheelEvent(0f, mouse.ScrollWheels[0].Y);
        }
        
        // mouse events
        foreach (InputMouseEvent mouseEvent in Engine.Input.MouseEvents)
        {
            io.AddMouseButtonEvent(mouseEvent.Button.ToImGuiMouseButton(), mouseEvent.Pressed);
        }

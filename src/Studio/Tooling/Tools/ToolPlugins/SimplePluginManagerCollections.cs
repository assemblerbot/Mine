...
public class SimplePluginManagerCollections
{
    ...
    public string Update(SimplePluginManagerSettings settings)
    {
        ...
        error_message = m_Repository.Init(repository_path);
        ...
        error_message = m_Project.Init(Path.Combine(Application.dataPath, project_path));
    ...
    public void CopyDependenciesFromRepositoryToProject(SimplePluginManagerPlugin plugin, SimplePluginManagerSettings settings)
    {
        ...
        string error_message = Update(settings);
        if (!string.IsNullOrEmpty(error_message))
        {
            ConsoleViewModel.Log($"Dependencies were NOT copied due to following error: {error_message}", ConsoleItemType.Error);
            return;
        }
